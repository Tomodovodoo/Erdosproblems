...
\hypersetup{
  ...
  pdftitle={The 7/27 Independence Bound},
  pdfsubject={Euclidean minimum-distance graphs}
}
...
\title{Extending from Swanepoel, proving $m=7$ and pushing the bound to\\ $\boldsymbol{g(n)\ge \lceil 7n/27\rceil}$}
\author{Tom de Groot, GPT 5.5 Pro \& Codex}
\date{May 2026}
...
\begin{abstract}
A Euclidean minimum-distance graph is the unit-distance graph of a finite point set in $\RR^2$ whose pairwise distances are all at least $1$. Let $g(n)$ be the minimum, over all such graphs on $n$ vertices, of the independence number. This document gives a complete proof, relative to Swanepoel's published broken-lattice inputs, that every Euclidean minimum-distance graph $G$ satisfies
\[
...
\]
The proof specializes Swanepoel's broken-lattice input to $m=7$, isolates the unique equality break, proves the required boundary-side orientation, and eliminates every remaining local configuration by explicit independent-set certificates.
\end{abstract}
...
Relative to the Swanepoel inputs stated in Proposition~\ref{prop:SwanepoelInput}, every Euclidean minimum-distance graph $G$ satisfies
\[
  \alpha(G)\ge \frac{7}{27}|V(G)|.
\]
Consequently,
...
\begin{remark}
The proof below proves the theorem relative to Proposition~\ref{prop:SwanepoelInput}. The endpoint use of $q_1$ and $q_9$ uses Lemma~8 in the range $1\le i\le 2m-5$. The cap for $t_5,t_6,t_7$ is the cap obtained in the proof of Lemma~9, not a separate theorem statement. The uniform side formula for $q_i$ is not imported from Swanepoel; it is proved internally in Lemma~\ref{lem:boundary-side}.
\end{remark}
...
\[
 \alpha(G)\ge \frac{7}{27}|V(G)|.
\]
If $\varnothing\ne S\subseteq V(G)$ is independent and
\[
...
\[
 G'=G-(S\cup N(S)).
\]
Since $S\ne\varnothing$, the graph $G'$ has fewer vertices than $G$. By minimality of $G$,
\[
 \alpha(G')\ge \frac{7}{27}|V(G')|.
\]
...
\section{The \texorpdfstring{$m=7$}{m=7} broken-lattice setup}
...
\]
Since the triple $(1,2,3)$ cannot be true, at least one equality fails. Since at most one equality fails, exactly one equality fails. To make all three forbidden triples false, the single failure must meet all three triples. Their only common index is $3$. Hence
\[
 s_3\ne r_4.
\]
\end{proof}
...
\section{Boundary-side orientation}

\begin{lemma}[Straight-line planarity]\label{lem:straight-planar}
...
\begin{lemma}[Empty unit triangle]\label{lem:empty-triangle}
...
\begin{lemma}[Boundary-side orientation]\label{lem:boundary-side}
Let $p_ip_{i+1}$ be one of the boundary triangle edges in the Swanepoel arc, and let $q_i$ be the unique common neighbour of $p_i$ and $p_{i+1}$. Orient the boundary cycle so that the interior of the drawing is locally on the left of each directed boundary edge in the arc. Then $q_i$ lies on the interior side of the directed edge $p_ip_{i+1}$. Consequently, after choosing the complex coordinate orientation so that left rotation by $60^\circ$ is multiplication by $\omega=e^{i\pi/3}$,
\[
...
\[
 |a-b|=|a-q|=|b-q|=1.
\]
Thus $a,b,q$ form a unit equilateral triangle, and $q$ is one of the two equilateral apices on the segment $ab$.

The segment $ab$ is an outer-boundary edge of the straight-line plane drawing. One side of $ab$ is incident with the outer face, and the other side is the interior side of the drawing. Suppose, for contradiction, that $q$ lies on the outer-face side of $ab$. Then the triangular cycle $a q b a$ lies on the outer-face side of $ab$. By Lemma~\ref{lem:empty-triangle}, its closed triangular region contains no other graph vertex. By Lemma~\ref{lem:straight-planar}, no graph edge crosses its boundary. Therefore the bounded triangular region enclosed by $a,q,b$ is a face of the plane embedding. On that side the edge $ab$ is then incident with this bounded triangular face, not with the outer face. This contradicts that $ab$ is an outer-boundary edge with the outer face on that side.
...
Throughout, identify $\RR^2$ with $\CC$ according to Lemma~\ref{lem:boundary-side}, and put
\[
 \omega=e^{i\pi/3}.
\]
Write
\[
 ...
 \qquad |e_i|=1.
\]
By Lemma~\ref{lem:boundary-side},
\[
 q_i=p_i+\omega e_i.
\]
...
\section{Cap lemmas and flat-port geometry}
...
In the fully flat branch, every extra neighbour of $t_i$, $i=5,6,7$, outside the strip-neighbour set lies in the $t_i$-port. If the port is saturated, its only port-neighbours are the two endpoints. If the port is not saturated, it contributes at most one extra neighbour. If one endpoint is already present and the port is not saturated, it contributes no further extra neighbour. Consecutive saturated ports share one endpoint.
\end{lemma}
...
\section{The \texorpdfstring{$t_4t_5$}{t4t5}-absent branch}
...
\section{The \texorpdfstring{$t_4t_5$}{t4t5}-present branch before full flatness}
...
\[
 \Re(x_6-p_3)>1+\frac12+\frac12+3\cdot\frac12=\frac72>1.
\]
For $p_0$,
\[
...
Assume that $G$ is a smallest counterexample. By Lemma~\ref{lem:deletion}, every nonempty independent set $S$ in $G$ must satisfy
\[
 |N(S)|>\left\lfloor\frac{20|S|}{7}\right\rfloor.
\]
By Lemma~\ref{lem:unique-break}, the unique equality break is $s_3\ne r_4$, and $t_i$ is defined for $i=1,2,4,5,6,7,8$.
